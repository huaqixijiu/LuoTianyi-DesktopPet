...
{
    public const double IconSize = 32, IconGap = 8, GroupWidth = 46, PanelWidth = 208, PanelHeight = 252, PanelGap = 8;
    ...
    public static MessageRailLayout Place(DesktopRectangle contour, double earY, DesktopRectangle work,
        MessageRailSide side, int iconCount, int rows, int activeIcon, double scale = 1,double? panelHeight = null)
    {
        ...
        double ph = Math.Min(Numeric.Clamp(panelHeight??PanelHeight,68,PanelHeight)*scale,
            Math.Max(1, work.Height - 8 * scale));
